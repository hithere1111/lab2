...
// Daniel Valvano
// June 19, 2013
// Code Edited by Bryan Brumm on 1/19/2014
...
#define DISPLAY_Y_MIDDLE        80          // 160/2 = vertical midpoint for two halves of a display
#define LINE_Y_LENGTH           20          // Vertical length in pixels for each line
#define LINE_Y_MID              10          // Vertical midpoint for each line
#define LINE_SIZE_MULT          1           // Size of each character for each line

#define LONG_DIGITS             10          // Number of digits in a long number
#define NULL                     0
...
void ST7735_InitR(enum initRFlags option) {
  ...
  ST7735_FillScreen(ST7735_WHITE);  // make screen background white
                                    // draw black horizontal line at pixel 80 to split the display into 2
  ST7735_DrawFastHLine(0, DISPLAY_Y_MIDDLE, _width, ST7735_BLACK);
}
...
//------------ST7735_ClearDisplay---------
// Clear top or bottom screen
// Input: [device] 1 bottom and 0 top
// Output: none
void ST7735_ClearDisplay(int device) {
   ST7735_FillRect(0, device*DISPLAY_Y_MIDDLE, _width, _height/2, ST7735_WHITE);
   ST7735_DrawFastHLine(0, DISPLAY_Y_MIDDLE, _width, ST7735_BLACK);
}
...
// Input: color 16-bit color, which can be produced by ST7735_Color565()
// Output: none
void ST7735_ClearLine(int device, int line) {
   ST7735_FillRect(0, device*DISPLAY_Y_MIDDLE+(line*LINE_Y_LENGTH), _width, LINE_Y_LENGTH, ST7735_WHITE);
   ST7735_DrawFastHLine(0, DISPLAY_Y_MIDDLE, _width, ST7735_BLACK);
}
...
// Converts each digit of a long value into an ASCII character
// The first digit will start at index 0 and after the last digit
// array will be null-terminated
static char* findDigits(long value, char digits[]) {
                                                         /******* I need to handle negative values at some point *******/
   int i = 1;
   while(value != 0){
      digits[i] = '0' + value%10; // adding the ASCII '0' to an integer 0-9
      value = value/10;
      i = i+1;
   }
   digits[0] = i;
...
//---------------ST7735_Message--------------
// Displays integer value on a specified line on the top or bottom of the LCD
// Each line has a vertical pixel length of 20, and characters are printed out
// starting at the midpoint of each length.
// Input: [device] 0-top, 1-bottom; [line] 0-top line, 3-bottom line;
// [*string] null terminated string; [value] integer to be displayed  
// Output: none
void ST7735_Message (int device, int line, char *string, long value) {
   unsigned short display = device*DISPLAY_Y_MIDDLE;           // starting point for vertical display
   short xPos = 0;                                             // starting point for horizontal display
   short yPos = display + (line*LINE_Y_LENGTH) + LINE_Y_MID;   // gives midpoint y-position for line being used
   char digitArray[LONG_DIGITS+1];                             // where the digits of value will be stored
   int i = 0;
   
   /* print the ASCII string */
   if(string != NULL){
      for(i=0;string[i] != NULL; i++){
         ST7735_DrawChar(xPos, yPos, string[i], ST7735_BLACK, ST7735_WHITE, 1);
         xPos = xPos + 5;
      }
   }
   
   if(value != NULL){
      char *digits = findDigits(value, digitArray);            // number of digits (including NULL) located at index 0
      unsigned char numDigits = digits[0];
	
      /* size of characters being printed cannot be greater than 20 pixels vertically */
      for(i=1;i<numDigits; i=i+1){
         ST7735_DrawChar(xPos, yPos, digits[numDigits-i], ST7735_BLACK, ST7735_WHITE, 1);
         xPos = xPos + 5; // values cannot overlap
      }
	}
}
