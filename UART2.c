...
void InterpretString(char *input) {
	...
	position = getNumber(input, 'a', &aValue);
	for(int i = 0; i < position; i++) {
		input++;
	}
	// Need checck for adc range
	
	position =  getNumber(input, 'c', &channel);
	for(int i = 0; i < position; i++) {
		input++;
	}
	...
	position = getNumber(input, 'f', &fsValue);
	for(int i = 0; i < position; i++) {
		input++;
	}
	// need to check for frequency values
	
	position = getNumber(input, "s", &samples);
	for(int i = 0; i < position; i++) {
		input++;
	}
	// need to check for accurate sample values
	
	
	// MAKE CALL TO ADC COLLECT HERE
	// SHOULD BE GOOD TO GO
	
	
}

int getNumber(char *input, char check, int *value) {
	int number;
	int *numbers;
	int sizeofnum = 0;
	int multiplier = 1;
